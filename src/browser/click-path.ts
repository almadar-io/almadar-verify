...
async function sampleOneSite(
  ...
  for (const selector of selectorsFor(site.event)) {
    ...
    if (!statePassed) {
      ...
      // EventLogEntry's `type` field is the event key (canonical core
      // shape post-state-bridge update). Older callers used `.event`
      // before the wire types hoist; `type` is the source of truth.
      const eventFired = eventLog.some((e) => e.type === site.event);
